...
public class ListSightingsOperation extends BaseOperation {
	...
	public void execute() {
		try {
			...
			List<BirdSightingData> list = ClientBuilder.newClient().target(getServerURL()).path("sighting").path(name)
					.request().get(new GenericType<List<BirdSightingData>>() {
					});
			Logger.getLogger(ListBirdsOperation.class.getName()).log(Level.INFO,
					String.format("BirdSightings table%n%s%n%s", getTableHeader(), getTableContent(list)));
		} catch (IOException e) {
